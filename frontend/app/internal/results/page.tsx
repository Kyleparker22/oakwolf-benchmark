...
function InternalResultsContent() {
  const params = useSearchParams();
  const router = useRouter();
  const uuid = params.get('uuid') || '';
  const mode = 'internal' as const;
  const [stage, setStage] = useState<'results'>('results');
  ...
  return (
    <main style={{ minHeight:'100vh', background:'#FAF9F6', padding:'2rem 1rem' }}>
      <div style={{ maxWidth:'760px', margin:'0 auto' }}>
        ...
        {/* Internal nav */}
        <div style={{ background:'#0F1F3D', borderRadius:'16px', padding:'1rem 1.5rem', marginBottom:'1.5rem', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'0.75rem' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
            <img src="/oakwolf-logo.jpg" alt="Oakwolf" style={{ height:'32px', filter:'brightness(0) invert(1)', opacity:0.85 }} />
            <span style={{ color:'rgba(255,255,255,0.35)', fontSize:'0.8rem' }}>{clientName}</span>
          </div>
          <div style={{ display:'flex', gap:'0.75rem', flexWrap:'wrap' }}>
            <button onClick={() => router.push('/internal/dashboard')} style={{ background:'rgba(255,255,255,0.1)', border:'none', color:'rgba(255,255,255,0.6)', padding:'0.4rem 0.875rem', borderRadius:'8px', cursor:'pointer', fontSize:'0.8rem', fontFamily:'DM Sans, sans-serif' }}>← Dashboard</button>
            <button onClick={() => router.push(`/internal/proposal?uuid=${uuid}`)} style={{ background:'linear-gradient(135deg, #0D9488, #0F766E)', border:'none', color:'white', padding:'0.4rem 1rem', borderRadius:'8px', cursor:'pointer', fontSize:'0.8rem', fontWeight:'600', fontFamily:'DM Sans, sans-serif' }}>📄 Generate Proposal</button>
            <button onClick={() => window.print()} style={{ background:'white', border:'none', borderRadius:'8px', padding:'0.4rem 0.875rem', fontSize:'0.8rem', fontWeight:'600', color:'#374151', cursor:'pointer', fontFamily:'DM Sans, sans-serif' }}>⬇ PDF</button>
          </div>
        </div>
        ...
        <div style={{ background:'white', border:'1px solid #E5E3DE', borderRadius:'20px', padding:'1.75rem', marginBottom:'1.5rem', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'1rem', boxShadow:'0 1px 4px rgba(0,0,0,0.04)' }}>
          <div>
            <h3 style={{ fontFamily:'DM Serif Display, serif', fontSize:'1.1rem', color:'#0F1F3D', fontWeight:'400', margin:'0 0 4px' }}>Benchmark the client's current setup</h3>
            <p style={{ fontSize:'0.825rem', color:'#6B6977', margin:0 }}>Have the client benchmark their internal team or current vendor against Oakwolf in 10 questions.</p>
          </div>
          <button onClick={() => router.push(`/vendor-benchmark?uuid=${uuid}`)} style={{ background:'linear-gradient(135deg, #0D9488, #0F766E)', border:'none', color:'white', borderRadius:'10px', padding:'0.75rem 1.25rem', fontWeight:'600', fontSize:'0.875rem', cursor:'pointer', fontFamily:'DM Sans, sans-serif', whiteSpace:'nowrap' }}>
            Benchmark Vendor or Internal FTE Team →
          </button>
        </div>
